<template>
  <div>
    <button type="button" @click="toggle()" :class="{on: light}">
...
export default {
  ...
  async created(){
    const state = await this.fetch('/state')
    this.light = state.light
  },

  computed: {
    display(){
      return this.light === true ? 'OFF' : 'ON'
    }
  },

  methods: {
    async toggle(){
      const res = await this.fetch('/toggle')
      this.light = res.light
    },

    async fetch(url){
      const isDev = document.location.hostname !== '192.168.1.201'
      const host = isDev ? 'http://192.168.1.201' : ''
...
button.on svg{
  filter: drop-shadow(0 0 5px #08F7FE);
}
</style>
